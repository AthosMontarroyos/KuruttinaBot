...
- **Time Duration Parsing & `parseTimeString` Component**: Commands accepting time durations (timeouts, mutes, message deletion, tempbans, reminders) MUST parse input via `parseTimeString(input, options)` (`src/utils/time/time-parser.ts`). Supports units `s`, `m`, `h`, `d` (e.g. `28s`, `28m`, `28h`, `7d`), fallback `defaultUnit` for raw numbers, flexible argument order in prefix commands (`k!cmd @user 7d motivo` or `k!cmd @user motivo 7d`), and localized `humanReadable` outputs.
- **Interaction Commands Gender & Asset Resolution Helper (`resolveInteractionAsset`)**: All interaction commands (kiss, hug, etc.) MUST resolve asset images/GIFs through `resolveInteractionAsset({ action, initiatorGender, targetGender })` (`src/utils/interactions/interaction-resolver.ts`), automatically filtering directory paths (`het/female_initiated`, `het/male_initiated`, `het/mutual`, `yuri`, `yaoi`) based on the users' genders and fallback rules.
